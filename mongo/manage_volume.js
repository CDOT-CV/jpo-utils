...
const MONGO_DATABASE_NAME = process.env.MONGO_DATABASE_NAME || "CV";

// The name of the collection to store the data as.
const MONGO_DATABASE_STORAGE_COLLECTION_NAME = process.env.MONGO_DATABASE_STORAGE_COLLECTION_NAME || "MongoStorage";

// Total Size of the database disk in GB. This script will work to ensure all data fits within this store.
const MONGO_DATABASE_SIZE_GB = process.env.MONGO_DATABASE_SIZE_GB || 1000;

// Specified as a percent of total database size. This is the storage target the database will try to hit. 
const MONGO_DATABASE_SIZE_TARGET_PERCENT = process.env.MONGO_DATABASE_SIZE_TARGET_PERCENT || 0.8;

// Specified as a percent of total database size.
const MONGO_DATABASE_DELETE_THRESHOLD_PERCENT = process.env.MONGO_DATABASE_DELETE_THRESHOLD_PERCENT || 0.9;

// The maximum amount of time data should be retained. Measured in Seconds.
const MONGO_DATABASE_MAX_TTL_RETENTION_SECONDS = process.env.MONGO_DATABASE_MAX_TTL_RETENTION_SECONDS || 5184000; // 60 Days
...
function compactCollections(){
	...
	for (var i = 0; i < collections.length; i++) {
		...
		if(freeSpace > allocatedStorage * 0.5 && allocatedStorage > (1 * BYTE_TO_GB)){
